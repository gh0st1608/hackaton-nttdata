...
async function bootstrap(): Promise<express.Express> {
  ...
  const app = await NestFactory.create(ValidateModule, new ExpressAdapter(expressApp));
  app.setGlobalPrefix('validate');
  app.enableCors();
...
export const authHandler = functions.http('validateHandler', async (req, res) => {
